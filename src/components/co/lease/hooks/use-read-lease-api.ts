...
const fetchData = (
  ...
  return axiosPrivate.get(`/lease/get-occupant-lease-list/${chiefOccupantId}`);
};

const getOccupantLeaseQueryOptions = (chiefOccupantId: string) => {
  return queryOptions({
    queryKey: ["get-occupant-lease-list", chiefOccupantId],
    queryFn: () => fetchData(chiefOccupantId),
  });
};

export const useGetOccupantLeaseList = () => {
